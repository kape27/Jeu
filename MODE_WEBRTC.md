...
- Cote hote: bouton `Copier le lien` pour partager une URL pre-remplie (session + serveur).
- Le champ serveur de signalisation est memorise localement dans le navigateur.
- Les parametres URL `?mode=bluetooth&role=join&session=XXXXXX&signal=wss://...` ouvrent directement l ecran de jonction pre-rempli.
